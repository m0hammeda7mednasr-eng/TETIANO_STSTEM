-- ====================================
-- إصلاح مشكلة عدم ظهور البيانات بعد ربط Shopify - نسخة محدثة
-- Fix Data Display Issue After Shopify Connection - Fixed Version
-- ====================================
...
BEGIN
    ...
    IF first_user_id IS NOT NULL AND default_store_id IS NOT NULL THEN
        ...
        -- إصلاح الطلبات وتوحيد الحالة المالية
        UPDATE orders 
        SET 
            user_id = COALESCE(user_id, first_user_id),
            store_id = COALESCE(store_id, default_store_id),
...
SET 
    current_total_price = COALESCE(current_total_price, 
        CASE WHEN total_price ~ '^[0-9]+\.?[0-9]*$' THEN total_price::DECIMAL ELSE 0 END, 0),
    total_refunded = CASE 
        WHEN financial_status = 'refunded' AND (total_refunded = 0 OR total_refunded IS NULL) 
        THEN CASE WHEN total_price ~ '^[0-9]+\.?[0-9]*$' THEN total_price::DECIMAL ELSE 0 END
        WHEN financial_status = 'partially_refunded' AND (total_refunded = 0 OR total_refunded IS NULL)
        THEN CASE WHEN total_price ~ '^[0-9]+\.?[0-9]*$' THEN (total_price::DECIMAL * 0.3) ELSE 0 END
...
SET cost_price = CASE 
    WHEN cost_price = 0 OR cost_price IS NULL THEN
        CASE 
            ...
            THEN (price::DECIMAL * 0.6) -- 60% من سعر البيع
